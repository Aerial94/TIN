...
void HTTPHandler::parse(std::string json) {
    ...
    if (!parsingSuccessful) {
        ...
        HTTPHandlerException httpHandlerException(
            HTTPHandlerException::Type::REQUEST_SYNTAX_ERROR);
    ...
    if (!root["command"].empty())
        ...
        HTTPHandlerException httpHandlerException(
            HTTPHandlerException::Type::REQUEST_NO_COMMAND);
        httpHandlerException.setDescription(
            "No command was specified in json file");
        Logger::getInstance().logWarning("HTTPHandler", "Invalid input json");
        throw httpHandlerException;
    }
    ...
    if (!root["domains"].empty()) {
        ...
        HTTPHandlerException httpHandlerException(
            HTTPHandlerException::Type::REQUEST_NO_DOMAINS);
        httpHandlerException.setDescription(
            "No domains were specified in json file");
        Logger::getInstance().logWarning("HTTPHandler", "Invalid input json");
        throw httpHandlerException;
    }
}
...
void HTTPHandler::chooseAction(std::string &json) {
    ...
    if (this->command == addAction) {
        ...
        for (auto it = this->domains.begin(); it != this->domains.end(); ++it) {
            ...
            this->pair["domain"] = *it;
            HTTPHandler::MessageStatus status =
                Database::getInstance().removeDomain(*it);
            this->pair["status"] = statusToString(status);
            this->vecOfDomainStatusPairs.append(this->pair);
            Logger::getInstance().logInfo(
                "HTTPHandler",
                "The following domains were removed: " + getAllDomainNames());
        }
    }
        ...
        if (this->star) {
            ...
            for (auto it = copyDomains.begin(); it != copyDomains.end(); ++it) {
                ...
                Logger::getInstance().logInfo("HTTPHandler",
                                              "All domains were queued");
            }
        }
        else {
            for (auto it = this->domains.begin(); it != this->domains.end();
                 ++it) {
                this->vecOfDomains.append(*it);
                Json::Value pair;
                pair["domain"] = *it;
                std::pair<HTTPHandler::MessageStatus, Domain> result =
                    Database::getInstance().getDomainStatus(*it);
                pair["status"] = statusToString(result.first);
                if (result.second.getStatus() ==
                    Domain::DomainStatus::FOLLOWED) {
                    pair["ip"] = result.second.getIP();
                }
                pair["timestamp"] = (unsigned int)result.second.getTimestamp();
                this->vecOfDomainStatusPairs.append(pair);
                Logger::getInstance().logInfo(
                    "HTTPHandler", "The following domains were queued: " +
                                       getAllDomainNames());
            }
        }
    }
...
std::string HTTPHandler::getAllDomainNames() {
    ...
    for (std::vector<std::string>::iterator it = this->domains.begin();
