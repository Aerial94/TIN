...
std::pair<HTTPHandler::MessageStatus, Domain> Database::getDomainStatus(
        std::string dName) {
    ...
    if ((domain = unsafeFindDomain(dName)) != domains.end() &&
        ...
        std::pair<HTTPHandler::MessageStatus, Domain> p = std::make_pair(status,
                                                                         *domain);
        return p;
    }
...
std::vector<Domain>::iterator Database::unsafeFindDomain(
